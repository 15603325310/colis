...
public class ModOverlap
{
	...
	public static void main(String args[])
	{
		...
		System.out.println((new StringBuilder()).append("模块度是 ").append(d).toString());
	}
	...
	private static HashMap readClusters(String s)
	{
		...
		{
			System.out.println((new StringBuilder()).append("社团文件错误").append(exception.toString()).toString());
			System.exit(1);
		}
		return hashmap2;
	}
	...
	public static HashMap readGraphEdges(String s)
	{
		...
		try
		{
			...
			{
				System.out.println((new StringBuilder()).append("文件").append(s).append("未找到").toString());
				System.exit(1);
			}
		}
		catch (Exception exception)
		...
		for (Iterator iterator = hashmap.keySet().iterator(); iterator.hasNext();)
		{
			...
			while (iterator2.hasNext()) 
			{
				...
				if (hashmap1.get(s) == null)
					System.out.println((new StringBuilder()).append("顶点").append(s).append("在网络中没有发现").toString());
